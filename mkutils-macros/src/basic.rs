...
impl Basic {
    fn field_assignment(field: &Field, method: &Ident) -> Result<TokenStream2, SynError> {
        let Some(field_name) = &field.ident else {
            return Err(Error::c_struct_field_missing_name(field.span()));
        };
        let field_assignment = quote::quote! { #field_name: self.#field_name.#method(&other.#field_name) };

        Ok(field_assignment)
    }

    fn value_for_c_struct(fields_named: &FieldsNamed, method: &Ident) -> Result<TokenStream2, SynError> {
        let field_assignments = fields_named
            .named
            .iter()
            .map(|field| Self::field_assignment(field, method))
    ...
    fn value_for_tuple_struct(fields_unnamed: &FieldsUnnamed, method: &Ident) -> TokenStream2 {
        let num_fields = fields_unnamed.unnamed.len();
        let field_values = (0..num_fields)
            .map(Index::from)
            .map(|index| quote::quote! { self.#index.#method(&other.#index) });
    ...
    fn value(input: &DeriveInput, method: &Ident) -> Result<TokenStream2, SynError> {
        let Data::Struct(data_struct) = &input.data else {
            return Err(Error::unsupported_item_type(input.span()));
        };
        let value = match &data_struct.fields {
            Fields::Named(fields_named) => Self::value_for_c_struct(fields_named, method)?,
            Fields::Unnamed(fields_unnamed) => Self::value_for_tuple_struct(fields_unnamed, method),
    ...
    fn derive_impl(input: &DeriveInput, trait_path: &str, method: &str) -> Result<TokenStream2, SynError> {
        ...
        let value = Self::value(input, &method)?;
